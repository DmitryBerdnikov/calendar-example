...
- Organizer может просматривать mocked bookings на `/bookings`, видеть event type title, guest name/email, organizer-local time range and status.
- Organizer может отменить confirmed booking через confirmation modal.
- Already cancelled bookings остаются видимыми, но cancel action disabled.
- Добавлен Playwright smoke для list и cancel.
